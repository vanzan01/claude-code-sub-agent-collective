...
describe('Registry Persistence and Recovery Tests', () => {
  ...
  describe('Backup and Recovery', () => {
    test('should create and restore from backups', async () => {
      ...
      const backupDir = path.join(testDir, 'state', 'backups');
      const backupFiles = await fs.readdir(backupDir);
      const registryBackup = backupFiles.find(f => f.startsWith('registry-backup-'));
      expect(registryBackup).toBeDefined();

      // Verify backup contents
      const backupData = await fs.readJson(path.join(backupDir, registryBackup));
      expect(backupData.agents).toHaveLength(1);
      expect(backupData.agents[0].id).toBe('backup-test-agent');

      await registry.shutdown();
    });

    test('should enforce backup limits', async () => {
      const registry = createRegistry({
        registryDir: path.join(testDir, 'state'),
        persistenceEnabled: true,
      ...
      const backupDir = path.join(testDir, 'state', 'backups');
      const backupFiles = await fs.readdir(backupDir);
      const registryBackups = backupFiles.filter(f => f.startsWith('registry-backup-'));
      
      expect(registryBackups.length).toBeLessThanOrEqual(2);
  ...
  describe('Performance and Scalability', () => {
    test('should handle large registry efficiently', async () => {
      const registry = createRegistry({
        registryDir: path.join(testDir, 'state'),
        persistenceEnabled: true
      });
      ...
      for (let i = 0; i < agentCount; i++) {
        registerPromises.push(registry.register({
          id: `perf-agent-${i}`,
          name: `Performance Agent ${i}`,
          template: i % 3 === 0 ? 'base' : i % 3 === 1 ? 'research-agent' : 'implementation-agent',
          path: `/test/path/agent${i}.md`,
          metadata: {
            registeredAt: new Date().toISOString(),
            version: '1.0.0'
      ...
      const activeAgents = registry.query({ status: 'active' });
      ...
      expect(activeAgents.length).toBe(agentCount);

      // Performance assertions
      expect(registrationTime).toBeLessThan(5000); // 5 seconds
      expect(queryTime).toBeLessThan(100); // 100ms
      expect(persistTime).toBeLessThan(1000); // 1 second

      console.log(`Performance metrics for ${agentCount} agents:`);
      console.log(`Registration: ${registrationTime}ms`);
      console.log(`Query: ${queryTime}ms`);
      console.log(`Persistence: ${persistTime}ms`);

      await registry.shutdown();
    }, 15000);

    test('should load large registry quickly', async () => {
      // First create a large registry
      let registry = createRegistry({
        registryDir: path.join(testDir, 'state'),
        persistenceEnabled: true
      });
      ...
      const loadedAgents = registry.query();
      expect(loadedAgents.length).toBe(agentCount);

      // Performance assertion
      expect(loadTime).toBeLessThan(2000); // 2 seconds

      console.log(`Load performance for ${agentCount} agents: ${loadTime}ms`);
