...
if echo "$TOOL_OUTPUT" | grep -q 'HANDOFF_TOKEN:[[:space:]]*[A-Z0-9_]\+'; then
    ...
    echo "█                                                                              █"
    echo "█  YOU MUST IMMEDIATELY EXECUTE THIS EXACT TASK TOOL CALL:                    █"
    echo "█                                                                              █"
    echo "█  Task(                                                                       █"
    echo "█    subagent_type='$TARGET_AGENT',                                           █"
    echo "█    description='Implement user request via routing',                        █"
    echo "█    prompt='create a todo application using html, css, js'                   █"
    echo "█  )                                                                           █"
    echo "█                                                                              █"
    echo "█  🚨 DO NOT RESPOND WITH TEXT - EXECUTE TASK TOOL IMMEDIATELY 🚨            █"
    echo "█                                                                              █"
    echo "████████████████████████████████████████████████████████████████████████████████"
    echo ""
